...
# Assume working directory is project root (AS_rainfall)
HPT_BASE = os.path.join('Hyperparameter_Tuning')
HYPERPARAM_DIR = os.path.join(HPT_BASE, 'output')

def load_best_hyperparameters(base_output_dir: str):
    """
    Load the best hyperparameters file from the tuning output directory.

    This supports both the new flat layout and the older nested layout:
      1) <base_output_dir>/land_model_cv_tuning/current_best_hyperparameters.py
      2) <base_output_dir>/land_model_extended_tuner/land_model_cv_tuning/current_best_hyperparameters.py

    Parameters
    ----------
    base_output_dir : str
        Base directory containing tuner project folders. Typically
    ...
    """
    candidates = [
        os.path.join(base_output_dir, 'land_model_cv_tuning', 'current_best_hyperparameters.py'),
        os.path.join(base_output_dir, 'land_model_extended_tuner', 'land_model_cv_tuning', 'current_best_hyperparameters.py'),
    ]
    ...
    if path is None:
        raise FileNotFoundError(
            "Hyperparameters file not found. Looked for: "
            + " | ".join(candidates)
        )
...
def build_model(data_metadata, hp_dir: str = os.path.join(HPT_BASE, 'output_1'), hyperparams=None):
    """
    ...
    local_dem = layers.Dense(
        units=hyperparams['local_dem_units'], 
        activation=hyperparams['activation'],
        kernel_regularizer=regularizers.l2(l=hyperparams['l2_reg'])
    )(local_dem)
    ...
    regional_dem = layers.Dense(
        units=hyperparams['regional_dem_units'], 
        activation=hyperparams['activation'],
        kernel_regularizer=regularizers.l2(l=hyperparams['l2_reg'])
    )(regional_dem)
    ...
    climate_flat = layers.Reshape(target_shape=(data_metadata['climate_shape'][0] * 
                                  data_metadata['climate_shape'][1] * 
                                  data_metadata['climate_shape'][2],))(climate_input)
    
    climate = layers.Dense(
        units=hyperparams['climate_units'], 
        activation=hyperparams['activation'],
        kernel_regularizer=regularizers.l2(l=hyperparams['l2_reg'])
    )(climate_flat)
    ...
    concat = layers.Concatenate(axis=-1)([climate, local_dem, regional_dem, month])
    
    # Dense layers
    x = layers.Dense(
        units=hyperparams['na'], 
        activation=hyperparams['activation'],
        kernel_regularizer=regularizers.l2(l=hyperparams['l2_reg'])
    )(concat)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(rate=hyperparams['dropout_rate'])(x)
    ...
    output = layers.Dense(units=1, activation=output_activation, name='rainfall')(x)
