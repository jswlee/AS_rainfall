...
class LANDModel(nn.Module):
    ...
    def __init__(self, 
        ...
        if climate_processing == 'conv2d':
            # Note: 3x3 kernel on 3x3 input with no padding produces 1x1 output
            # Grouped convolution here is primarily a parameter-sharing scheme.
            # If climate_units is divisible by in_ch, use groups=in_ch (one group per variable)
            # to match the baseline diagram.
            in_ch = climate_shape[0]
            out_ch = climate_units
            if out_ch % in_ch != 0:
                raise ValueError(
                    f"climate_units ({out_ch}) must be divisible by num_climate_vars ({in_ch}) when using grouped Conv2D."
                )
            groups = in_ch
        ...
        # Third dense stage for climate branch
        self.climate_fc3 = nn.Linear(in_features=self._climate_units_out, out_features=self._climate_units_out)
        self.climate_bn3 = nn.BatchNorm1d(num_features=self._climate_units_out)
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Third dense stage for climate branch
        climate_out = self.climate_fc3(climate_out)
        climate_out = self.climate_bn3(climate_out)
        climate_out = self.climate_activation_fn(climate_out)
...
def create_model_from_hyperparams(hyperparams: Dict, metadata: Dict) -> LANDModel:
    ...
    required_keys = [
        ...
        'na', 'nb', 'dropout_rate', 'use_residual',
