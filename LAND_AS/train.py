...
def main():
    ...
    save_json({"station_roles": bundle.metadata["roles"], "year_ranges": bundle.metadata["year_ranges"]}, output / "split.json")

    for fold_number, (train_index, val_index) in enumerate(cv_folds(bundle, args.folds)):
        ...
        for seed_number in range(args.seeds):
            ...
            save_json({**history, "best_val_mae_mm": score}, fold_output / f"seed_{seed}_history.json")
